...
export function registerAllHandlers(win: BrowserWindow): void {
  ...
  // --- Clipboard ---
  const clipboardImagePath = path.join(os.tmpdir(), 'rig-clipboard.png');
  ...
  ipcMain.handle(IPC.SetDockIcon, (_e, args: { dataUrl: string }) => {
    if (process.platform !== 'darwin' || !app.dock) return;
    try {
      const img = nativeImage.createFromDataURL(args.dataUrl);
      if (!img.isEmpty()) app.dock.setIcon(img);
    } catch (e) {
      console.warn('[dock icon] Failed to set icon:', e);
    }
  });
