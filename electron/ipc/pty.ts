...
export function spawnAgent(
  ...
  const containerName = args.dockerMode ? `rig-${args.agentId.slice(0, 12)}` : null;
...
export const DOCKER_DEFAULT_IMAGE = 'rig-agent:latest';

/** Label key used to stamp the Dockerfile content hash on built images. */
const DOCKERFILE_HASH_LABEL = 'rig-dockerfile-hash';
...
/**
 * Check if a project has a local Dockerfile at .rig/Dockerfile.
 * Returns the absolute path if found, null otherwise.
 */
export function resolveProjectDockerfile(projectRoot: string): string | null {
  const p = path.join(projectRoot, '.rig', 'Dockerfile');
  try {
