...
describe('resolveProjectDockerfile', () => {
  it('returns absolute path when .rig/Dockerfile exists in project root', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'pty-resolve-'));
    tempPaths.push(projectRoot);
    const dockerDir = path.join(projectRoot, '.rig');
    fs.mkdirSync(dockerDir, { recursive: true });
    fs.writeFileSync(path.join(dockerDir, 'Dockerfile'), 'FROM node:20\n');

    const result = resolveProjectDockerfile(projectRoot);
    expect(result).toBe(path.join(projectRoot, '.rig', 'Dockerfile'));
  });

  it('returns null when .rig/Dockerfile does not exist', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'pty-resolve-'));
    tempPaths.push(projectRoot);
  ...
  it('returns null when .rig/Dockerfile is a directory', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'pty-resolve-'));
    tempPaths.push(projectRoot);
    fs.mkdirSync(path.join(projectRoot, '.rig', 'Dockerfile'), { recursive: true });
...
describe('projectImageTag', () => {
  it('returns a tag in the format rig-project:<12-char-hash>', () => {
    ...
    expect(tag).toMatch(/^rig-project:[a-f0-9]{12}$/);
  });

  it('returns rig-project:unknown for non-existent Dockerfile path', () => {
    const tag = projectImageTag('/nonexistent/Dockerfile');
    expect(tag).toBe('rig-project:unknown');
  });
});
...
describe('dockerImageExists', () => {
  it('fails closed when a custom dockerfile path is unreadable', async () => {
    ...
    await expect(
      dockerImageExists('rig-project:test', {
