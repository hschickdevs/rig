...
const TRUST_PATTERNS: RegExp[] = [
  ...
  /confirm.*folder.*trust/i, // "Confirm folder trust" — TUI variant (normal and garbled)
];
 ...
 * screen frame.  TUI agents (e.g. Codex CLI) redraw the full screen on
...
const QUESTION_PATTERNS: RegExp[] = [
  ...
  /trust.*folder/i,
  // TUI header: "Confirm folder trust" (normal and TUI-garbled "Confirmfoldertrust").
 ...
 *
 *  For full-screen TUI agents (e.g. Codex CLI) that clear+redraw their
...
export function looksLikeQuestion(tail: string): boolean {
  ...
  // TUI agents (e.g. Codex CLI) use cursor positioning instead of
...
// banner a TUI agent may emit before entering the alternate screen.
...
export function markAgentOutput(agentId: string, data: Uint8Array, taskId?: string): void {
  ...
  if (looksLikePrompt(lastLine)) {
    ...
    //
    // NOTE: do NOT cancel pendingAnalysis here.  TUI agents (e.g. Codex CLI)
    // use Ink which positions the ❯ selection cursor in a separate
