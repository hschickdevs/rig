...
describe('normalizeCurrentFrame', () => {
  ...
  it('returns the same value for consecutive redraws of identical content', () => {
    // This is the key property: Ink-based TUI agents redraw the same screen on every
...
describe('looksLikeQuestion', () => {
  ...
  it('returns false when prompt marker is not on the last line (footer below prompt)', () => {
    // Some TUI agents render a status bar / footer after the ❯ input cursor, so ❯
    // ends up one line above the last non-empty line.  Should still clear state.
    const tail =
      'Would you like to initialize agent instructions?\n❯ \nMode: interactive | Claude Sonnet 4.5';
    expect(looksLikeQuestion(tail)).toBe(false);
  });
  ...
  it('returns false when prompt marker is at end of TUI-garbled line', () => {
    // TUI cursor-positioning can concatenate the init suggestion and the ❯
    // input cursor onto the same stripped line when no actual newlines are used.
    const tail = 'Would you like to initialize agent instructions?❯ ';
    expect(looksLikeQuestion(tail)).toBe(false);
  });

  it('returns true for trust dialog content ending with ❯ (buffer truncation mid-frame)', () => {
    // When the PTY buffer captures a TUI frame mid-write, the tail can
  ...
  it('returns true for trust dialog rendered as single TUI string ending with ❯', () => {
    // TUI cursor-positioning collapses everything to one string.
  ...
  it('returns false after screen clear even when old question text precedes it', () => {
    // Full-screen TUI agents emit \x1b[2J before every redraw.
  ...
  it('detects trust dialog rendered via cursor-positioning (no newlines)', () => {
    // Ink-based TUI agents use cursor-positioning sequences (\x1b[N;1H) and
    ...
    const ESC = '\x1b';
    // Simulate TUI rendering: cursor-positioned lines, no \r\n
...
describe('isTrustQuestionAutoHandled', () => {
  ...
  it('returns true for "Confirm folder trust" TUI dialog when autoTrustFolders is enabled', () => {
    mockAutoTrustFolders = true;
    expect(isTrustQuestionAutoHandled('Confirm folder trust\n❯ Yes  No')).toBe(true);
  });

  it('returns true for garbled trust dialog when autoTrustFolders is enabled', () => {
    mockAutoTrustFolders = true;
