...
import { updateDockIcon } from './lib/dockIcon';
import {
  store,
  loadAgents,
  loadState,
...
function App() {
  ...
  // Repaint the macOS Dock icon to match the active theme. No-op on Linux/Windows.
  createEffect(() => {
    if (!isMac) return;
    const look = store.themePreset;
    void updateDockIcon(look).catch((err: unknown) => {
      console.warn('[dock icon] update failed', err);
    });
  });
