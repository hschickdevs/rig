...
- **Corner radius:** macOS Big Sur+ expects a specific squircle (~22.37% of icon size). Don't use a plain `rounded-rect` — either use Apple's template or let `electron-builder` handle it from a square 1024×1024 source.
...
build/icon.svg                    ← design source for bundled icon (1024×1024, squared)
build/icon-rounded.svg            ← rounded variant for preview
...
build/icon.icns                   ← macOS bundle (regenerated by electron-builder)
...
1. Edit `build/icon.svg` (the design source)
2. Export to `build/icon.png` at 1024×1024 — from Figma, Sketch, or:
   ```sh
   rsvg-convert -w 1024 -h 1024 build/icon.svg > build/icon.png
   ```
3. `npm run build` — `electron-builder` regenerates `.icns`, `.ico`, and sized PNGs
4. The built app bundle picks them up automatically
